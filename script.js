...
highlightListItems();
console.log("list items turned to blue!");
...
function toggleStatus(event) {
    ...
    if(statusOutput.classList.contains("hidden")){
        //added this so that the timestamps reset and don't keep appending to each other
        statusOutput.innerHTML = "Current Status Information: <br>"; 
...
console.log("ToggleStatus loaded!");
...
function stopFlashing() {
    ...
    intervalId = null;
    alert("flashing has ended!");
}

function startFlashing() {
    if (intervalId !== null) return; // its already flashing 
    intervalId = setInterval(() => {
        controlPanel.classList.toggle("hidden");
    }, 500);
    alert("flashing has begun!");
}
